...
void main() {
  ...
  group('تطابق جدول المهن مع بذرة الخادم', () {
    // القائمة الحيّة ما عادت في الشيفرة: المالك يحرّرها من اللوحة وتُقرأ من
    // قاعدة البيانات، فمهنة يضيفها اليوم موجودة على الخادم وغير موجودة هنا
    // — وهذا صحيح ومقصود، والتطبيق يجيبها من `GET /professions`.
    //
    // الذي يبقى واجباً تطابقه هو **البذرة**: القائمة التي تبدأ بها أي قاعدة
    // بيانات جديدة (constants/professions.ts) وهذي النسخة المحلية، لأن هذي
    // هي الأرضية التي يتعرّف عليها التطبيق حين تنقطع الشبكة فما يصل
    // المصنّف. انحرافها عواقبه صامتة: مهنة في البذرة وناقصة عندنا تُصنَّف صح
    // عبر الـLLM ثم تسقط بلا اسم عربي، ومهنة عندنا وناقصة هناك تُطابَق
    // محلياً ثم يرفضها الخادم.
    ...
    test('نفس السلوقات ونفس الأسماء ونفس الصيغ في البذرة', () {
