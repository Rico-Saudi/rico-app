...
/// قائمة المهن المعروفة محلياً — نسخة من **بذرة** الخادم
/// (server/rico-backend/src/professionals/constants/professions.ts).
///
/// ما عادت القائمة الكاملة: المهن صارت صفوفاً يحرّرها المالك من لوحة
/// التحكم، والقائمة الحيّة تجي من `GET /professionals/professions` (انظر
/// [ProfessionalsService.fetchProfessions]) — فالمنتقي يفضّلها دائماً، وأي
/// مهنة يضيفها المالك اليوم تظهر في أجهزة منصّبة من زمان بلا تحديث.
///
/// وظيفة هذي النسخة اثنتان: الاحتياط حين تفشل الشبكة، وكونها **المصدر
/// الوحيد** للمطابقة المحلية بالكلمات المفتاحية في [IntentService] — تلك
/// تحتاج الصيغ العامية ([Profession.aliases]) التي لا يرسلها الخادم أصلاً.
/// أي أنها الأرضية: ما يتعرّف عليه التطبيق بلا إنترنت، لا سقف ما يعرفه.
///
